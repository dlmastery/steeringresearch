...
| E36 | SAE selection problem | E | A7 (how derived) | SAE-feature steering underperforms raw DiffMean unless features are output-score-selected; the AxBench tension is a selection problem, not a representation problem. | SAE-steer naive vs selected vs DiffMean; selected SAE matches DiffMean [NEEDS VERIFICATION] | **WEAK/WASH on real data** (synthetic-only SUPPORTED did not generalize): diffmean~=pca at matched fractional alpha (C9b); WEAK/WASH on REAL AxBench (2B, L20, 20 concepts × 8 eval, alpha=0.10, exp#124): pca+add behavior 0.178 vs diffmean+add 0.169 (+0.009 margin) but pca coherence 0.644 vs diffmean coherence 0.681 (−0.037) — a tradeoff not a win; source barely matters; PCA marginally higher behavior, DiffMean marginally higher coherence; despite divergent directions on real concepts (E4, S-19), steering outcomes similar (S-21) | 3 | — |
...
| N5 | Norm-budget conservation law | Novel | A3+A12 | There is a conserved edit budget B ~ q-quantile(||h||); coherence collapses when ||sum alpha_i v_i|| > B regardless of how it is spent. | Re-plotting E17-E22 against ||delta h||/||h|| produces a single master collapse curve [NEEDS VERIFICATION] | **FALSIFIED at rung-3 (across scale)** — held-out R2 = -1.6 when predicting 1B from 270M coefficients, i.e. the *universal law* claim fails; the monotone direction survives but the coefficients are model-specific (see FINDINGS.md and STATUS.md). Screening evidence that originally suggested it: logPPL=5.40+2.87*offshell R2=0.81 (C2, 23 rows). Superseded — do not cite as SUPPORTED. | 2 | `ideas/_campaigns/` |
...
| M8 | Conformal gate with a PROVABLE over-refusal guarantee | G | A5 (condition) | Split-conformal calibration of the per-intent cosine gate guarantees benign over-refusal <= alpha (distribution-free, finite-sample) on a held-out in-distribution set, while retaining harmful recall >= 0.70 at alpha=0.01 and Pareto-dominating a classifier-router on {over-refusal, latency, paraphrase-robustness}. | Realized in-dist over-refusal <= alpha + 0.02 (coverage); harmful recall >= 0.70 at alpha=0.01; Pareto-dominates router on >= 1 axis; held-out calibration (fixes S-25 in-sample-tau); n >= 7 bootstrap. | DESIGN + PLAN — RUN PENDING. Blocked on M1 (real refusal direction) + judge AUC >= 0.85 precondition. Fixes the S-25 in-sample-tau confound. The ownable contribution (lab review item #9). | Rung 3 NOT STARTED | hypotheses/B_conditional/M8_conformal_gate.md. Score-agnostic coverage guarantee turns the moderate AUC-0.74 gate (S-25) into a CERTIFIED selectivity control; uses raw cosine (not the overfit probe, S-26). |
