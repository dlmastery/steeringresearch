...
2. **Single-seed report at the best config.** Metric variance across
   seeds is non-trivial at low epoch budgets. The 3-seed confirmation
   gate is mandatory before any external claim.
...
5. **Hill-climbing a broken hypothesis.** If the critic audit gave a
...
6. **Tuning the screening config in place.** The screening baseline must
...
8. **Hill-climbing on the most expensive evaluation rung directly.**
