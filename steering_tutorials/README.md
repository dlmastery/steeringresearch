...
flowchart TD
    ...
    click D8 "streaming_trajectory_aggregation/README.md"
...
| [`streaming_trajectory_aggregation`](streaming_trajectory_aggregation/README.md) | early/streaming detection of scheming WITHIN a long agent trajectory (SHADE, median 126 steps): an 8-method aggregator ladder (true mean vs max vs GRU vs learned query-token compression) plus two **causal** streaming monitors, with a directly computable lead time (`point_of_no_return_step − ideal_flagging_step`) — the streaming/many-step sequel to `trajguard` (streaming) + `cross_trajectory` (aggregation) | [Trajectory Guard (arXiv:2601.00516)](https://arxiv.org/abs/2601.00516) · [Tracing the Dynamics of Refusal / SALO (arXiv:2605.02958)](https://arxiv.org/abs/2605.02958) · [Real-Time Detection and Repair of LLM Agent Failures (arXiv:2608.02464)](https://arxiv.org/abs/2608.02464) · [SafetyDrift (arXiv:2603.27148)](https://arxiv.org/abs/2603.27148) · [TRACE — Risk-Aware Compression (arXiv:2606.00611)](https://arxiv.org/abs/2606.00611) · [TRACE — Cross-Step Evidence Aggregation (arXiv:2606.07054)](https://arxiv.org/abs/2606.07054) — full table incl. 2 DO-NOT-CITE traps in [`CITATIONS_VERIFIED.md`](streaming_trajectory_aggregation/CITATIONS_VERIFIED.md) | 🧪 SHADE run landed 2026-08-17 (screening tier) — 4/5 applicable falsifiers HOLD, **F4 fails hard**; AgentDojo (F2) in flight |
...
| [`streaming_trajectory_aggregation`](streaming_trajectory_aggregation/README.md) · DEFEND | **mean-pooling really does dilute at 126 steps — and it changes nothing, because a bag-of-words rival matches the best deployable monitor** (SHADE, n=500/class, screening tier, 5 CV groups): true `mean_pool` **0.740** vs `max_pool` **0.858** vs `gru` **0.905**, but the content confound bar is **0.9039**, so `gru` clears it by **0.0012** with a CI that *straddles* the bar; the only clear winner (`query_token_compressor` 0.939) is **non-causal** and undeployable. Both **causal streaming monitors sit at chance** (0.511 / 0.459) — F4 **FAILS**. The 11-step median lead is over **3 of 132** positives at a **0.294** false-alarm rate — a hollow pass. Note it **contradicts `cross_trajectory`** above, whose `mean_agg` is `[mean,max,std]`, not a mean pool |
