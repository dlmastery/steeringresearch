...
| Results honesty | **PASS — and the run is no longer pending** | **This row previously read "[PENDING GPU RUN]"; that is stale.** `artifacts/results.json` is a complete run: both regimes (`0.05` and `0.5`) carry AP + 95% CI + ROC-AUC for all three localizers, `clustering.best_k = 16` (by silhouette over k ∈ {4,6,8,10,12,16}), `cluster_purity 0.955`, and a populated `ood` block over `intrinsec-ai/cstm-bench` (52 attack / 56 benign). **The pre-registered falsifier FIRES:** at the 5% sparse rate `kmeans_enrich` AP **0.199** [0.118, 0.285] loses to `per_trace` AP **0.731** [0.616, 0.838], so the paper's clustering ordering is **FALSIFIED on this reconstruction** — the README leads with that negative result rather than the 0.5-regime numbers where every method looks strong (0.893–0.972). OOD is at chance across the board (per-trace ROC-AUC 0.540, kmeans_enrich 0.424, knn_purity 0.500). Screening-tier disclosed (one embedder, one seed fraction, a few hundred traces); no LLM judge (detection lesson); length confound audited and quoted beside the APs; adaptive-dispersion weakness flagged as the paper's own. **Caveat this row must carry:** the artifact's two embedder fields disagree — `embed_model: "BAAI/bge-base-en-v1.5"` beside `embedder: "minilm"` — and every number above was produced with **MiniLM**. The paper's own encoder (`bge`, and the `config.py` default) has still never been run. |
