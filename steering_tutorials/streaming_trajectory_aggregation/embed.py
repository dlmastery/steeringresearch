...
def cache_path(embedder_id: str, corpus_name: str, n_per_class: int, seed: int) -> Path:
    key_src = "%s|%s|%d|%d" % (embedder_id, corpus_name, int(n_per_class), int(seed))
...
def load_or_build_embeddings(corpus: Corpus, embedder_id: str = None, *,
    ...
    if loaded is not None:
        ...
        if ok and len(cached) == len(corpus.trajectories):
            _eprint("[embed] cache HIT: %s (n=%d, dim=%d, fingerprint verified)"
                    % (path, len(cached), cached_dim))
            return cached, cached_dim
        detail = _describe_mismatch(want, got_meta)
        if len(cached) != len(corpus.trajectories):
            detail += "; trajectory count %d vs %d" % (len(cached), len(corpus.trajectories))
...
if __name__ == "__main__":
    ...
    try:
        ...
        # regression: same trajectory COUNT, different step TEXT -> must be refused.
        ...
        print("OK -- embed.py self-test: ragged pack round-trips exactly, fingerprint "
              "REJECTS a same-count/different-text cache instead of returning stale "
              "vectors under new content (CLAUDE.md section 18.8).")
    finally:
