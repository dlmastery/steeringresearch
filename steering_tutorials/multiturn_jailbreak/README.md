...
attacks generated by **ActorAttack** ("Derail Yourself", Ren et al. 2024,
arXiv:2410.10700 — [UNVERIFIED]), which decomposes a harmful goal into a chain of
...
> `artifacts/results.json` after the run. The table is wired to the schema (rows
> = 4 methods × 2 embedders); until then the cells read `PENDING`.
...
(DeepContext, arXiv:2602.16935 — [UNVERIFIED]; Hierarchical Attention for
multi-turn safety, arXiv:2606.21082 — [UNVERIFIED]).

**Gemma embedder** (layer 12):

| method | AUC | F1 | TPR@FPR=0.10 |
|---|---|---|---|
| `per_turn_max` (stateless baseline) | PENDING | PENDING | PENDING |
| `trajectory_mlp` | PENDING | PENDING | PENDING |
| `seq_gru` | PENDING | PENDING | PENDING |
| `hier_attn` | PENDING | PENDING | PENDING |

**MiniLM embedder** (384-d sentence embedding):
...
**Confound audit** (from `results.json.confound`): `turncount_auc` = PENDING,
`totalchar_auc` = PENDING, `turncount_pos_mean` = PENDING, `turncount_neg_mean` =
PENDING. Read these **first**: an AUC near 0.5 licenses reading the method table
as trajectory signal; well above 0.5 means discount the headline by the length
tell.

**Pre-registered falsifier.** The lesson's thesis is the ordering
`AUC(seq_gru) > AUC(per_turn_max)`. **If `seq_gru` AUC ≤ `per_turn_max` AUC**
(within the bootstrap CI) on the Gemma embedder, the "trajectory matters" claim
is **FALSE for this setup** — the escalation carried no signal the stateless
baseline could not already grab from the single most-suspicious turn, and no
amount of narration rescues it. We report that outcome as prominently as a
success.
...
- **Negatives are topic-matched UltraChat, not paired twins.** These are hard,
  same-surface benign chats — but they are *sampled*, not per-attack benign twins
  of the exact same subject. A benign twin set would be a stronger control; that
  is future work.
- **The confound is audited, not assumed away.** If `turncount_auc` /
  `totalchar_auc` come back well above 0.5, part of the method AUC is a length
  tell, and the headline is discounted accordingly — the audit is reported next to
  the result, never buried.
